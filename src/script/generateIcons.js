...
const svgOriginalColors = [
  "icon-alert-danger.svg",
  "icon-alert-info.svg",
  "icon-alert-sucess.svg",
  "icon-alert-warning.svg",
  "icon-arrow-tooltip.svg",
  "icon-arrow2-down.svg",
  "icon-arrow2-left.svg",
  "icon-arrow2-right.svg",
  "icon-close.svg",
  "icon-home.svg",
  "icon-info-circle-tooltip.svg"
];
...
function convertSVGAttributes(svg) {
  ...
  let converted = svg.replace(/\bclass=/g, "className=");
...
const svgToComponent = (name, svgContent) => {
  ...
  svgContent = svgContent
    ...
    // Remove width/height/class/className da tag <svg> raiz para o
    // className passado pelo componente Icon ter controle total do tamanho
    .replace(/<svg([^>]*)>/, (_, attrs) => {
      const cleaned = attrs
        .replace(/\s+(width|height|class|className)="[^"]*"/g, "");
      return `<svg${cleaned}>`;
    })
